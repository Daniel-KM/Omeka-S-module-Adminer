...
class IndexController extends AbstractActionController
{
    ...
    protected function adminer(string $type)
    {
        ...
        // Strip server/username/db from all urls generated by Adminer.
        // The controller re-injects them into $_GET on every request, so
        // Adminer works fine without them in the browser url.
        $s = urlencode($authData['server']);
        $u = urlencode($authData['username']);
        $d = urlencode($authData['db']);
        $search = [
            "server=$s&amp;username=$u&amp;db=$d&amp;" => '',
            "server=$s&amp;username=$u&amp;db=$d" => '',
            "server=$s&username=$u&db=$d&" => '',
            "server=$s&username=$u&db=$d" => '',
        ];

...
class AdminerCleanUrls extends Adminer\Plugin
{
    ...
    public function headers(): void
    {
        ...
        ob_start(fn ($output) => strtr($output, $search));
    }

    public function head(): void
    {
        $params = json_encode(array_merge([Adminer\DRIVER, 'username', 'db'], $this->extraParams));
    ...
    protected $translations = [
        'cs' => ['' => 'Skryje přihlašovací údaje z adresního řádku prohlížeče'],
        'de' => ['' => 'Verbindungsparameter aus der Browser-Adressleiste ausblenden'],
        'fr' => ['' => 'Masquer les paramètres de connexion dans la barre d’adresse du navigateur'],
        'ja' => ['' => '接続パラメータをブラウザのアドレスバーから非表示'],
    ];
}
